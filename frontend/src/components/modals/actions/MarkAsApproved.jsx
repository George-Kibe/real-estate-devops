...
const MarkAsApproveddModal = ({ isOpen, onClose, currentProperty, userProperties, setUserProperties, currentIndex }) => {
  const [reason, setReason] = useState('');
  const markAsApproved = () => {
    const updatedProperty = { ...currentProperty, isApproved: { isApproved: currentProperty?.isApproved?.isApproved? false : true, reason: reason} };
  ...
  return (
    ...
    > 
      ...
      >
        ...
        <p className="font-semibold text-xl mb-4 pr-2">Mark Property as Approved  </p>
        <p className="">
          You are about to mark this proiperty as approved. This action will notify the relevant team.
        </p>
        ...
        <div className="flex flex-col justify-end md:flex-row gap-4">
          ...
          <Button onClick={markAsApproved} className="mt-2 bg-green-600 hover:bg-lime-500 rounded-xs">
            Mark Approved
          </Button>
        </div>
      </div>
...
export default MarkAsApproveddModal;