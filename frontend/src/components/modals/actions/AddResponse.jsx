...
const AddResponseModal = ({ isOpen, onClose, currentProperty, userProperties, setUserProperties, currentIndex }) => {
  const [response, setResponse] = useState('');
  const AddReponse = () => {
    const updatedProperty = { ...currentProperty, response: response };
  ...
  return (
    ...
    > 
      ...
      >
        ...
        <p className="font-semibold text-xl mb-4 pr-2">Add Property Response </p>
        <p className="">
          Add a Detailed Response of your communication with the Agent or the landlord.
        </p>
        <div className="flex flex-col gap-2 mt-4 ">
            <p className="font-semibold pr-2">
              Please provide a the Detailed Response
            </p>
            <textarea type="text" className="border-1" placeholder="e.g. property doesn't meet client criteria" value={response} onChange={(e) => setResponse(e.target.value)} />
        </div>
        <div className="flex flex-col justify-end md:flex-row gap-4">
          ...
          <Button onClick={AddReponse} className="mt-2 bg-red-600 hover:bg-lime-500 rounded-xs">
            Add Response
          </Button>
        </div>
      </div>
