...
const MarkAsDeniedModal = ({ isOpen, onClose, currentProperty, userProperties, setUserProperties, currentIndex }) => {
  const [reason, setReason] = useState('');
  const markDenied = () => {
    const updatedProperty = { ...currentProperty, denied: { denied: currentProperty?.denied?.denied? false : true, reason: reason} };
  ...
  return (
    ...
    > 
      ...
      >
        ...
        <p className="font-semibold text-xl mb-4 pr-2">Mark Denied </p>
        <p className="">You are setting this property status to Denied. This will notify the relevant parties that the property is not suitable and will be removed from the active listings.</p>
        ...
        <div className="flex flex-col justify-end md:flex-row gap-4">
          ...
          <Button onClick={markDenied} className="mt-2 bg-red-600 hover:bg-lime-500 rounded-xs">
            Mark Denied
          </Button>
        </div>
      </div>
...
export default MarkAsDeniedModal;