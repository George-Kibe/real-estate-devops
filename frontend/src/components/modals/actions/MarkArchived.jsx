...
const MarkAsArchivedModal = ({ isOpen, onClose, currentProperty, userProperties, setUserProperties, currentIndex }) => {
  const [reason, setReason] = useState('');
  const markArchived = () => {
    const updatedProperty = { ...currentProperty, archived: { archived: currentProperty?.archived?.archived? false : true, reason: reason} };
  ...
  return (
    ...
    > 
      ...
      >
        ...
        <p className="font-semibold text-xl mb-4 pr-2">Archive Property </p>
        <p className="">Archiving will remove this property from the active listings. You can restore it anytime from the archive section</p>
        ...
        <div className="flex flex-col justify-end md:flex-row gap-4">
          ...
          <Button onClick={markArchived} className="mt-2 bg-red-600 hover:bg-lime-500 rounded-xs">
            Archive
          </Button>
        </div>
      </div>
...
export default MarkAsArchivedModal;