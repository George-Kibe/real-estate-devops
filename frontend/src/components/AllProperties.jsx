...
function createArrayOfNumbers(n) {
    const array = [];
    for (let i = 1; i <= n; i++) {
        array.push(i);
    }
...
const AllProperties = () => {
    ...
    const fetchProperties = async () => {
        try {
          const {results, count} = await fetchDjangoProperties({page});
            ...
            <ul className="flex items-center justify-center gap-2">
                {
                    page > 1 && (
                        <li>
                            <button onClick={() => setPage(page - 1)} className="flex h-10 min-w-10 items-center justify-center rounded-lg border border-stroke bg-white px-2 text-base font-medium text-dark hover:bg-gray-1 dark:border-white/10 dark:bg-white/5 dark:text-white dark:hover:bg-white/10">
                                <span>
                                <svg
                                    width="20"
                                    height="21"
                ...
                }
                {
                    pages.map((p) => (
                        <li key={p}>
                            <button onClick={() => setPage(p)} className={`flex h-10 min-w-10 items-center justify-center rounded-lg border border-stroke bg-white px-2 text-base font-medium text-dark hover:bg-gray-1 dark:border-white/10 dark:bg-white/5 dark:text-white dark:hover:bg-white/10 ${page === p ? 'bg-gray-1' : ''}`}>
                                {p}
                            </button>
                        </li>
                    ))
